...
from models.model_hybrid import LF_CNN, TF_CNN, PF_CNN
from util import *
import pickle
import xlwt
import wandb
            ...
            for subID in tqdm(range(1, 30, 2)):
                ...
                for seg in range(0, 33, 2):
                    ...
                    for train_index, test_index in kf.split(X_EEG):
                        ...
                        # 加载模型，配置训练参数
                        if modelName == 'LF':
                            model = LF_CNN(EEG_channel=30, NIRS_channel=36, rank=16).cuda()
                        elif modelName == 'TF':
                            model = TF_CNN(EEG_channel=30, NIRS_channel=36, rank=16).cuda()
                        elif modelName == 'PF':
                            model = PF_CNN(EEG_channel=30, NIRS_channel=36, rank=16).cuda()
